...
class AddProductSeoScreenState extends State<AddProductSeoScreen> {
  ...
  void initState() {
    ...
    if(_update) {
      ...
      Provider.of<SellerProvider>(context,listen: false).getProductImage(widget.product!.id.toString());
    }else {
      _product = Product();
  ...
  route(bool isRoute, String name, String type, String? colorCode) async {
    if (isRoute) {
      ...
      if(_update){
        ...
        if(Provider.of<SellerProvider>(Get.context!,listen: false).withColor.isNotEmpty) {
          for(int index=0; index<Provider.of<SellerProvider>(Get.context!,listen: false).withColor.length; index++) {
            String retColor = Provider.of<SellerProvider>(Get.context!,listen: false).withColor[index].color!;
            String? bb;
            if(retColor.contains('#')){
              bb = retColor.replaceAll('#', '');
            }
            if(bb == colorCode) {
              Provider.of<SellerProvider>(Get.context!,listen: false).setStringImage(index, name, colorCode!);
              break;
            }
          }
        }
        ...
        for(int i = 0; i< Provider.of<SellerProvider>(Get.context!,listen: false).withColor.length; i++){
          if(Provider.of<SellerProvider>(Get.context!,listen: false).withColor[i].image != null){
            withc++;
          }
        }

        for(int i = 0; i< Provider.of<SellerProvider>(Get.context!,listen: false).withoutColor.length; i++){
          if(Provider.of<SellerProvider>(Get.context!,listen: false).withoutColor[i].image != null){
            withOurC++;
          }
        }

        log("--===> Image is ==> ${Provider.of<SellerProvider>(Get.context!,listen: false).totalPickedImage} and ${Provider.of<SellerProvider>(Get.context!,listen: false).totalUploaded}");
        if(Provider.of<SellerProvider>(Get.context!,listen: false).totalUploaded >= (withc+withOurC)){
          Provider.of<SellerProvider>(Get.context!,listen: false).addProduct(context, _product!, _addProduct!, thumbnailImage, metaImage, !_update, tagList);
        ...
        if(Provider.of<SellerProvider>(Get.context!,listen: false).withoutColor.isNotEmpty) {
          for(int index=0; index<Provider.of<SellerProvider>(Get.context!,listen: false).withoutColor.length; index++) {
          }
        }
        ...
        if(metaImage ==''){
          total = Provider.of<SellerProvider>(Get.context!,listen: false).withColor.length+ Provider.of<SellerProvider>(Get.context!,listen: false).withoutColor.length + 1;
        }else{

          total = Provider.of<SellerProvider>(Get.context!,listen: false).withColor.length+ Provider.of<SellerProvider>(Get.context!,listen: false).withoutColor.length + 2;
        }
                ...
                return SingleChildScrollView(
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.start,
                    mainAxisAlignment: MainAxisAlignment.center, children: [
                          ...
                          return (context, sc, tags, onTagDelete) {
                            ...
                            return TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: Theme.of(context).primaryColor, width: 3.0)),
                                helperText: '',
                                helperStyle: robotoRegular.copyWith(color: Theme.of(context).primaryColor, fontSize: Dimensions.fontSizeDefault),
                                ...
                                prefixIcon: tags.isNotEmpty ? SingleChildScrollView(
                                  ...
                                  child: Row(children: tags.map((String? tag) {
                                        ...
                                        return Container(decoration: BoxDecoration(
                                            borderRadius: const BorderRadius.all(Radius.circular(20.0)), color: Theme.of(context).primaryColor),
                                          margin: const EdgeInsets.symmetric(
                                              horizontal: 5.0),
                                          padding: const EdgeInsets.symmetric(
                                              horizontal: 10.0, vertical: 5.0),
                                          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                              InkWell(child: Text('$tag',
                                                style:  robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault, color: Colors.white)),
                                                onTap: () {
                                                  //print("$tag selected");
                                                },),
                                              const SizedBox(width: 4.0),
                                              InkWell(child: const Icon(Icons.cancel, size: Dimensions.fontSizeDefault, color: Color.fromARGB(255, 233, 233, 233)),
                                                onTap: () {
                                                  onTagDelete(tag!);
                                                },
                                              )
                      ...
                      Text(getTranslated('meta_image', context)!, style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeDefault)),
                      const SizedBox(height: Dimensions.paddingSizeExtraSmall),
                      ...
                      Align(alignment: Alignment.center, child: Stack(children: [
                        ClipRRect(borderRadius: BorderRadius.circular(Dimensions.paddingSizeSmall),
                          ...
                          ) :widget.product!=null? CustomImage(
                        ...
                        Positioned(bottom: 0, right: 0, top: 0, left: 0,
                          child: InkWell(onTap: () => resProvider.pickImage(false,true, false, null),
                      ...
                      Align(alignment: Alignment.center, child: Stack(children: [
                        ClipRRect(borderRadius: BorderRadius.circular(Dimensions.paddingSizeSmall),
                          child: resProvider.pickedLogo != null ?  Image.file(File(resProvider.pickedLogo!.path),
                            width: 150, height: 120, fit: BoxFit.cover) :widget.product!=null?
                          CustomImage(placeholder: Images.placeholderImage,
                            image: '${Provider.of<SplashProvider>(context).configModel!.
                            baseUrls!.productThumbnailUrl}/${_product!.thumbnail ?? ''}',
                            height: 120, width: 150, fit: BoxFit.cover,
                          ): Image.asset(Images.placeholderImage,height: 120, width: 150, fit: BoxFit.cover)),

                        Positioned(bottom: 0, right: 0, top: 0, left: 0,
                          child: InkWell(onTap: () => resProvider.pickImage(true,false, false, null),
                            child: Container(decoration: BoxDecoration(color: Colors.black.withOpacity(0.3),
                                borderRadius: BorderRadius.circular(Dimensions.paddingSizeSmall),
                                border: Border.all(width: 1, color: Theme.of(context).primaryColor)),
                              child: Container(margin: const EdgeInsets.all(25),
                              ...
                              child: Stack(children: [
                                DottedBorder(
                                  ...
                                  child: ClipRRect(
                                    borderRadius: BorderRadius.circular(Dimensions.paddingSizeSmall),
                                    child: (_update)?
                                    CustomImage(image: "${Provider.of<SplashProvider>(context, listen: false).configModel?.baseUrls?.productImageUrl}/${resProvider.withColor[index].imageString}",
                                ...
                                return Stack(children: [
                                  DottedBorder(
                                    ...
                                    child: Container(decoration: const BoxDecoration(color: Colors.white,
                                        borderRadius: BorderRadius.all(Radius.circular(20)),),
                                      child: ClipRRect(borderRadius: const BorderRadius.all(Radius.circular(Dimensions.paddingSizeSmall)),
                                        child: CustomImage(image: "${Provider.of<SplashProvider>(context, listen: false).configModel?.baseUrls?.productImageUrl}/${resProvider.imagesWithoutColor[index]}",
                                          width: MediaQuery.of(context).size.width/2.3,
                                          height: MediaQuery.of(context).size.width/2.3,
                                          fit: BoxFit.cover)))),
              ...
              if (!_update && resProvider.pickedLogo == null) {
                ...
                if(_update){
                  ...
                  if((withc + withOurC) <= resProvider.totalUploaded && resProvider.pickedLogo == null && resProvider.pickedMeta == null){
                    Provider.of<SellerProvider>(context,listen: false).addProduct(context, _product!, _addProduct!, thumbnailImage, metaImage, !_update, tagList);
                  } else{
