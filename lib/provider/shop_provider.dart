...
import 'package:sixvalley_vendor_app/data/model/response/product_image_model.dart';
...
class SellerProvider extends ChangeNotifier {
  ...
  List<ImageModel> updateWithColor = [];
  List<ImageModel> updateWithoutColor = [];



  void pickImage(bool isLogo,bool isMeta, bool isRemove, int? index, {bool update = false}) async {
    if(isRemove) {
      totalPickedImage--;
      _pickedLogo = null;
      _pickedCover = null;
      ...
      if(type == 'product'){
        ...
        if(attributeList![0].active){
          ...
          if(update && map["color_image"]['color'] != null && index != null && (index < imagesWithColorForUpdate.length)){
            ...
            colorImageObject.insert(index, ColorImage(color:  map['color_image'] != null ? map['color_image']['color'] : null, imageName: name));
          }else{
            log("========not Here========");
  ...
  Future<void> addProduct(BuildContext context, Product product, AddProductModel addProduct, String? thumbnail, String? metaImage, bool isAdd, List<String?> tags) async {
    ...
    if(_variantTypeList.isNotEmpty) {
      ...
      if (kDebugMode) {
        print('=====Total Quantity======>$_totalQuantity/ ${productReturnImage?.toList()}/${colorImageObject.toList()}');
      }
    }
    ApiResponse response = await shopRepo!.addProduct(product,addProduct ,fields, productReturnImage, thumbnail, metaImage, isAdd, attributeList![0].active, colorImageObject, tags);
    if(response.response != null && response.response!.statusCode == 200) {
      _productCode.clear();
  ...
  Future<void> deleteProductImage(String id, String name, String? color) async {
    _isLoading = true;
    ApiResponse apiResponse = await shopRepo!.deleteProductImage(id, name, color);
    getProductImage(id);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      _isLoading = false;
  ...
  Future<void> getProductImage(String id) async {
    ...
    _isLoading = true;
    ApiResponse apiResponse = await shopRepo!.getProductImage(id);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      imagesWithoutColor = [];
      productReturnImage = [];
      imagesWithColorForUpdate =[];
